...
extension GameViewController: SaveStatesViewControllerDelegate {
    ...
    private func load(_ saveState: SaveStateProtocol) {
        ...
        var temporarySaveState: SaveStateProtocol?
...
private extension GameViewController {
    ...
    @objc func emulationDidQuit(with notification: Notification) {
        DispatchQueue.main.async {
            ...
            token = self.emulatorCore?.observe(\.state, options: [.initial]) { emulatorCore, _ in
...
private extension GameViewController {
    ...
    @objc func rewindPollFunction() {
        ...
        if let system = System(gameType: game.type), let identifier = system.deltaCore?.identifier {
            fetchRequest.predicate = NSPredicate(format: "%K == %@ AND %K == %@ AND %K == %@", #keyPath(SaveState.game), game, #keyPath(SaveState.coreIdentifier), identifier, #keyPath(SaveState.type), NSNumber(value: SaveStateType.rewind.rawValue))
        } else {
